...
class Attention_DIFF(nn.Module):
    """
    KPGT 特征注意力模型，用于分子对差值预测任务 (分层差值特征融合架构)
    该模型使用局部注意力作为 adapter 学习分子的 KPGT 特征，并使用分层差值作为输入
    将2304维特征拆分为3个768维子特征，分别应用注意力机制
    """

    def __init__(self, args):
        super(Attention_DIFF, self).__init__()
        ...
        # 差值预测网络（注意力加权融合架构）
        # 每个层级的差值特征维度
        per_level_dim = self.hidden_size
        
        # 注意力加权融合层
        self.attention = nn.Sequential(
            nn.Linear(per_level_dim, 1),  # 轻量全连接层
            nn.Softmax(dim=1)  # Softmax在层级维度上分配权重
        )
        
        # 增加BN层用于特征归一化
        self.bn = nn.BatchNorm1d(per_level_dim)
        
        # MLP输入维度：与单层级特征维度相同（因为注意力加权后是各层特征的加权和）
        hidden_dim = per_level_dim * 4  
        
        self.fc_layer = nn.Sequential(
            nn.Linear(per_level_dim, hidden_dim),  
    ...
    def forward(self, batch_smiles1, batch_smiles2):
        ...
        def process_local_attention(features):
            ...
            for i in range(self.num_subfeatures):
                ...
                # 通过注意力encoder，保存每个层级的特征
                attention_mask = None
                for level in range(self.SA_N):
                    projected, _ = self.subfeature_encoders[i][level](projected, attention_mask)
                    
                    # 保存当前层级的特征（移除序列维度）
                    level_feature = projected.squeeze(1)
                    level_features[level].append(level_feature)
                
            # 合并每个层级的子特征
            merged_level_features = []
            for level in range(self.SA_N):
                # 合并当前层级的所有子特征
                merged = torch.cat(level_features[level], dim=1)
                # 添加额外的正则化
                merged = self.extra_layer_norm(merged)
                merged_level_features.append(merged)
            
            return merged_level_features
        
        # 处理第一个分子特征，获取每个层级的特征
        level_features1 = process_local_attention(features1)
        
        # 处理第二个分子特征，获取每个层级的特征
        level_features2 = process_local_attention(features2)
        
        # 注意力加权差值特征融合
        # 计算每个层级的差值
        diff_features = []
        for level in range(self.SA_N):
            diff = level_features1[level] - level_features2[level]
            diff_features.append(diff)
        
        # 将所有层级的差值特征堆叠（batch_size, num_levels, hidden_size）
        diff_stack = torch.stack(diff_features, dim=1)
        
        # 计算每个层级的注意力权重
        attention_weights = self.attention(diff_stack)
        
        # 应用注意力权重（batch_size, hidden_size）
        fused_features = torch.sum(diff_stack * attention_weights, dim=1)
        
        # 应用BN层进行特征归一化
        F_concat = self.bn(fused_features)
        
        # 通过差值预测网络
        out = self.fc_layer(F_concat)
