...
class Attention_SNN(nn.Module):
    """
    KPGT 特征注意力模型，用于分子对差值预测任务 (SNN架构)
    该模型使用局部注意力作为 adapter 学习分子的 KPGT 特征，并使用差值作为输入
    将2304维特征拆分为3个768维子特征，分别应用注意力机制
    """

    def __init__(self, args):
        super(Attention_SNN, self).__init__()
        ...
        hidden_dim = target_dim * 2  # SNN架构调整：隐藏层大小减半
        
        self.fc_layer = nn.Sequential(
            nn.Linear(target_dim, hidden_dim),  # SNN架构调整：输入维度从 target_dim*2 改为 target_dim
